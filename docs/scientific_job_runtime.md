...
`20260807011500_durable_scientific_job_runtime_phase2c2a_rpc_ambiguity_repair.sql` is an additive repair for databases where the original migration was already applied. PostgreSQL treats `RETURNS TABLE` field names as PL/pgSQL variables, so the repair explicitly qualifies every queue-table column in the acquisition and outbox-dispatch RPCs. Apply migrations in version order; do not alter either migration in a deployed database.
...
`scripts/check_scientific_job_runtime_staging.py` provides destructive-safe acceptance modes for an explicitly designated staging project only: identity, schema, lease race, completion race, expiry recovery, attachment reconciliation, cancellation race, dead-letter, stale-outbox recovery, reconciliation race, and fixture cleanup. It refuses to run unless both `MYSTIC_STAGING_SUPABASE_PROJECT_REF` and the direct PostgreSQL host bind to its compiled staging project reference. It uses a fresh `psql` process for each operation, so competing worker calls are independent database sessions.

The harness requires a protected, dedicated staging database credential supplied outside source control. It never prints the connection URL, password, or lease capability; its fixtures have a fixed, cleanup-only prefix. Do not point it at production or substitute a project reference. Run `--cleanup` after acceptance testing.
