...
Phase 2D.2 supplies the versioned Wave 1 corpus, fixed provider serializers, local baseline command, result persistence, and evidence-derived activation gate. `python scripts/run_specialist_benchmarks.py` records the safe non-specialist baseline. Adding `--live` remains an operator-only action: it requires explicit server-side NVIDIA NIM configuration and rights-cleared, manually labelled real-document assets for OCR/table/layout evaluation. The local MCP and browser do not expose arbitrary provider/model execution.
