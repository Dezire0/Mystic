  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        provider: {
          type: "string",
          enum: ["auto", "openai_compatible", "gemini", "google_vertex_ai", "anthropic", "local_backend"],
        },
...
async function callCloudTool(name, args, env, state) {
  ...
  if (name === "provider_call_test") {
    ...
    return {
      ...
      auth_method: record.auth_method,
      status,
      message: providerStatusMessage({ ...record, status, provider_status: status }),
      setup_url: record.setup_url,
