...
interface Env {
  ...
  MYSTIC_CONSOLE_RATE_LIMITS: KVNamespace;
}
...
function isMutation(request: Request) { return ["POST", "PUT", "PATCH", "DELETE"].includes(request.method); }
function sameOriginMutation(request: Request, url: URL) { return !isMutation(request) || request.headers.get("origin") === url.origin; }
async function loginRateLimitKey(request: Request, env: Env) { return `login:${await hmac(env.MYSTIC_CONSOLE_SESSION_SECRET, request.headers.get("cf-connecting-ip") ?? "unknown")}`; }
async function loginAllowed(request: Request, env: Env) { const raw = await env.MYSTIC_CONSOLE_RATE_LIMITS.get(await loginRateLimitKey(request, env), "json") as { attempts?: number } | null; return (raw?.attempts ?? 0) < 5; }
async function recordLoginFailure(request: Request, env: Env) { const key = await loginRateLimitKey(request, env); const prior = await env.MYSTIC_CONSOLE_RATE_LIMITS.get(key, "json") as { attempts?: number } | null; await env.MYSTIC_CONSOLE_RATE_LIMITS.put(key, JSON.stringify({ attempts: (prior?.attempts ?? 0) + 1 }), { expirationTtl: 600 }); }
async function clearLoginFailures(request: Request, env: Env) { await env.MYSTIC_CONSOLE_RATE_LIMITS.delete(await loginRateLimitKey(request, env)); }
...
function normalizeScene(payload: unknown) {
  ...
  const revision = String(scene.revision ?? scene.updated_at ?? scene.created_at ?? "unknown");
...
async function sceneWithRevision(env: Env, sceneId: string, request: Request) { const scene = normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId })); const expected = request.headers.get("if-match"); if (!expected || expected !== scene.scene.revision) throw new ApiFailure("scene_conflict", "The scene was changed by another client.", 409); const revision = Number(expected); if (!Number.isInteger(revision) || revision < 1) throw new ApiFailure("scene_conflict", "The scene revision is invalid.", 409); return revision; }

async function routeApi(request: Request, env: Env, url: URL) {
  const pathname = url.pathname;
  if (!sameOriginMutation(request, url)) return error("unauthorized", "Cross-site requests are not accepted.", 403);
  if (pathname === "/api/auth/session") return json({ authenticated: await validSession(request, env) });
  if (pathname === "/api/auth/login" && request.method === "POST") {
    if (!(await loginAllowed(request, env))) return error("unauthorized", "Too many login attempts. Try again later.", 429);
    const body = z.object({ token: z.string().min(1), email: z.string().email().optional() }).safeParse(await request.json().catch(() => ({})));
    if (!body.success || !env.MYSTIC_CONSOLE_ADMIN_TOKEN || !textEqual(body.data.token, env.MYSTIC_CONSOLE_ADMIN_TOKEN) || !allowedEmail(env, body.data.email)) { await recordLoginFailure(request, env); return error("unauthorized", "Login was not accepted.", 401); }
  ...
  try {
    ...
    if (pathname === "/api/research" && request.method === "GET") { const result = await invoke(env, "lab_session_list", { limit: 50 }) as { records?: unknown[]; next_cursor?: string }; return json({ sessions: result.records ?? [], next_cursor: result.next_cursor ?? "" }); }
    ...
    if (pathname === "/api/scenes" && request.method === "GET") { const result = await invoke(env, "lab_scene_list", { limit: 50 }) as { records?: unknown[]; next_cursor?: string }; return json({ scenes: result.records ?? [], next_cursor: result.next_cursor ?? "" }); }
    ...
    if (pathname === "/api/activity" && request.method === "GET") { const result = await invoke(env, "lab_activity_list", { limit: 100 }) as { records?: unknown[]; next_cursor?: string }; return json({ events: result.records ?? [], next_cursor: result.next_cursor ?? "" }); }
    ...
    if (sceneMatch) { const sceneId = decodeURIComponent(sceneMatch[1]); const objectId = sceneMatch[2] ? decodeURIComponent(sceneMatch[2]) : undefined; const simulationId = sceneMatch[3] ? decodeURIComponent(sceneMatch[3]) : undefined;
      ...
      if (pathname.endsWith("/objects") && request.method === "POST") { const expectedRevision = await sceneWithRevision(env, sceneId, request); const body = objectInput.parse(await request.json()); await invoke(env, "add_lab_object", { scene_id: sceneId, object: backendObject(body.object), expected_revision: expectedRevision }); return json(normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId }))); }
      if (objectId && request.method === "PATCH") { const expectedRevision = await sceneWithRevision(env, sceneId, request); const body = patchInput.parse(await request.json()); await invoke(env, "update_lab_object", { scene_id: sceneId, object_id: objectId, patch: body.patch, expected_revision: expectedRevision }); return json(normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId }))); }
      if (objectId && request.method === "DELETE") { const expectedRevision = await sceneWithRevision(env, sceneId, request); await invoke(env, "remove_lab_object", { scene_id: sceneId, object_id: objectId, expected_revision: expectedRevision }); return json(normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId }))); }
      if (pathname.endsWith("/simulations") && request.method === "POST") { const expectedRevision = await sceneWithRevision(env, sceneId, request); const body = simulationInput.parse(await request.json()); const result = await invoke(env, "run_lab_simulation", { scene_id: sceneId, adapter_id: body.adapterId, inputs: body.inputs, expected_revision: expectedRevision }); return json({ simulation: result, scene: normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId })) }); }
      if (simulationId && request.method === "POST") { const expectedRevision = await sceneWithRevision(env, sceneId, request); await invoke(env, "attach_simulation_to_scene", { scene_id: sceneId, simulation_id: simulationId, apply_object_updates: true, expected_revision: expectedRevision }); return json(normalizeScene(await invoke(env, "get_lab_scene", { scene_id: sceneId }))); }
    }
