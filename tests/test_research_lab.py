...
from mystic.llm_client import LLMClientError
...
class ResearchLabTests(unittest.TestCase):
    ...
    @patch("mystic.research_lab.parallel_reasoning_enabled")
    @patch("mystic.research_lab.remote_reasoning_state")
    @patch("mystic.research_lab.load_model_defaults")
    @patch("mystic.research_lab.build_client")
    @patch("mystic.research_lab.load_dashboard_snapshot")
    def test_run_research_lab_falls_back_to_local_when_remote_rate_limited(
        self,
        snapshot_loader,
        client_builder,
        defaults_loader,
        ...
        remote_state.return_value = (True, "openai-compatible", "llama-3.3-70b-versatile")
        parallel_enabled.return_value = False
        ...
        critic_client = FakeClient(
            [
                '{"verdict":"VALID","first_fatal_error":"","missing_assumptions":[],"invalid_steps":[],"valid_steps":["ok"],"repair_possible":true,"confidence":0.8,"final_status":"VALID"}'
            ]
        )
        remote_prime_method = FakeClient(
            [LLMClientError("OpenAICompatibleClient request failed: 429 Client Error: Too Many Requests")]
        )
        remote_logic_method = FakeClient(
            ['{"method_summary":"logic은 완전성만 체크한다","task_candidate":"분기 누락 점검","dependencies":["prime"],"deliverable":"완전성 메모"}']
        )
        remote_prime_execution = FakeClient(
            [LLMClientError("OpenAICompatibleClient request failed: 429 Client Error: Too Many Requests")]
        )
        remote_logic_execution = FakeClient(
            ["UNDERSTANDING:\nlogic 이해\nSTRATEGY:\n완전성 점검 전략\nEXECUTION:\n누락을 확인했다\nCONCLUSION:\nlogic 결론 완성\nUNCERTAINTIES:\n없음"]
        )
        client_builder.side_effect = [
            local_client,
            ...
            remote_prime_method,
            remote_logic_method,
            remote_prime_execution,
            remote_logic_execution,
        ]
