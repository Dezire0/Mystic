...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Install and manage the Mystic web launchd agent.")
    ...
    install_parser = subparsers.add_parser("install", parents=[common], help="Write plist and start the Mystic web service.")
    install_parser.set_defaults(func=install_agent)

    start_parser = subparsers.add_parser("start", parents=[common], help="Start or restart the installed Mystic web service.")
    start_parser.set_defaults(func=start_agent)

    stop_parser = subparsers.add_parser("stop", parents=[common], help="Stop the Mystic web service.")
    stop_parser.set_defaults(func=stop_agent)

    restart_parser = subparsers.add_parser("restart", parents=[common], help="Restart the Mystic web service.")
    restart_parser.set_defaults(func=restart_agent)

    status_parser = subparsers.add_parser("status", parents=[common], help="Show Mystic web service status.")
...
def plist_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "StandardOutPath": str(logs_dir / "mystic_web.launchd.stdout.log"),
        "StandardErrorPath": str(logs_dir / "mystic_web.launchd.stderr.log"),
    }
...
def status_agent(args: argparse.Namespace) -> int:
    base_dir = Path(args.base_dir).resolve()
    logs_dir = base_dir / "logs"
    stdout_path = logs_dir / "mystic_web.launchd.stdout.log"
    stderr_path = logs_dir / "mystic_web.launchd.stderr.log"
    ...
    payload = {
        ...
        "healthcheck": healthcheck(args.host, int(args.port)),
        "mcp_endpoint": f"http://{args.host}:{args.port}/mcp",
    }
    print(json.dumps(payload, indent=2, ensure_ascii=True))
    return 0


def healthcheck(host: str, port: int) -> dict[str, Any]:
